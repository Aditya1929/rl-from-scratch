...
Contrast with the DQN agent next door:
  • DQN was OFF-policy: a 100k replay buffer, learn every step from old memories.
  • PPO is ON-policy: collect a fresh batch of experience with the CURRENT policy,
    do a few epochs of updates on exactly that batch, then THROW IT AWAY.

The three ideas that make PPO work, all visible below:
  1. GAE (Generalized Advantage Estimation) — a low-variance estimate of
     "how much better was this action than average?"  (compute_gae)
  2. The CLIPPED surrogate objective — the update is allowed to improve the
     policy, but the clip stops it from moving too far in one step. (update)
  3. An entropy bonus — keeps the policy from collapsing to deterministic
     too early, preserving exploration.
"""
import numpy as np
import torch
...
class PPOAgent:
    def __init__(
        ...
        epochs:       int   = 10,     # how many passes over each rollout
        minibatch:    int   = 64,
        value_coef:   float = 0.50,   # weight of the critic loss
        entropy_coef: float = 0.01,   # weight of the exploration bonus
    ...
    def select_action(self, state):
        ...
        IS the exploration; there is no epsilon here). Returns the action to send
        to the env plus the bookkeeping PPO needs later: the log-prob and value.
        """
        s = torch.as_tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
        dist   = self.net.get_dist(s)
        action = dist.sample()                       # may fall outside valid range
        logp   = dist.log_prob(action).sum(-1)       # log π(a|s)
        value  = self.net.get_value(s)
        # We store the RAW sampled action (and its log-prob). The env clips it to
        # valid ranges; keeping the raw action makes the log-prob bookkeeping exact.
    ...
    def compute_gae(self, last_value):
        ...
        λ trades off bias vs variance. The (1 − done) mask cuts the chain at
        episode boundaries so reward never leaks across resets.
    ...
    def update(self, last_value):
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, n, self.minibatch):
                ...
                # ratio = π_new(a|s) / π_old(a|s).  In log space that's a subtract
                # then exp. If the new policy makes a good action (adv>0) much more
                # likely, ratio grows — the clip caps how much we reward that in a
                # single update, preventing a destructive jump.
                ...
                # keeps the policy stochastic / exploring.
